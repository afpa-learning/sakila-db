...
delimiter //
CREATE OR REPLACE TRIGGER `stop_delete_admin` BEFORE DELETE ON staff FOR EACH ROW
BEGIN
    -- Insérer ici le code de vérification d'identité
    if OLD.staff_id = 1 then
    
    	SELECT concat('Impossible de supprimer l''utilisateur : ', old.last_name, ' ' , old.first_name) INTO @message;
